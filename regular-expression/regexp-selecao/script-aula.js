...
'JavaScript'.replace(regexpJava, 'Type');
// TypeScript
...
'Perdeu perdido'.replace(regexpI, 'Ba');
// Bardeu Bardido
...
const regexpCLASS = /[ai]/gi;

'JavaScript'.replace(regexpCLASS, 'u');
// JuvuScrupt
...
// Procura: - ou .
const regexpCLASS_ES = /[-.]/g;

'111.222-333-44'.replace(regexpCLASS_ES, '');
// 11122233344
...
'JavaScript é a linguagem.'.replace(regexpAZ, '0');
// J000S00000 é 0 000000000.

// Busca por itens de a à z e A à Z
const regexpAZM = /[a-zA-Z]/g;

'JavaScript é a linguagem.'.replace(regexpAZM, '1');
...
const regexpNegar = /[^a-z]/g;

'Brasil é com z: Brazil'.replace(regexpNegar, ' ');
// rasil   com z   razil 
...
'JavaScript é a linguagem.'.replace(regexpPonto, '0');
...
'999.222.222.11'.replace(regexpES, '-');
// 999-222-222-11
...
'Guarda-chuva R$ 23,00.'.replace(regexpWord, '-');
// ------------ -$ --,--.
...
const regexpNotWord = /\W/g;

'Guarda-chuva R$ 23,00.'.replace(regexpNotWord, '-');
...
const regexpNotD = /\D/g;

'+55 (21) 2222-2222'.replace(regexpNotD, '');
// 552122222222
...
'+55 (21) 2222-  2222  '.replace(regexpSpace, '');
// +55(21)2222-2222
...
const regexpNSpace = /\S/g;

'+55 (21) 2222-  2222  '.replace(regexpNSpace, '');
// XXX XXXX XXXXX  XXXX  
...
const regexpQ = /aaaa/g;
const regexpAlt = /a{4}/g;

'Vaaaai ali por favor.'.replace(regexpQ, 'a');
// Vai ali por favor.  
...
const regexpQMM = /\d{2,3}/g;

'222.333.222.42'.replace(regexpQMM, 'X');
// X.X.X.X
...
'222.333.222.42'.replace(regexpM, 'X');
// X.X.X.X

// Procura: Começa com d, seguido por uma ou mais letras.
const regexpLetrasM = /d\w+/g;

'Dígitos, dados, desenhos, Dito, d'.replace(regexpLetrasM, 'X');
// Dígitos, X, X, Dito, d
...
const regexpAs = /d\w*/g;

'Dígitos, dados, desenhos, Dito, d'.replace(regexpAs, 'X');
// Dígitos, X, X, Dito, X
...
'Qual é o certo, regexp ou regex?'.replace(regexpOP, 'Regular Expression');
...
const regexpALT = /java|php/gi;

'PHP e Java são linguagens diferentes'.replace(regexpALT, 'X');
...
const regexpWB = /java/gi;
'Java não é JavaScript.'.replace(regexpWB, 'X');
// X não é XScript.
...
const regexpDigitoNW = /\B\d+\B/gi;

'11_22 33-44 55é66 77e88'.replace(regexpDigitoNW, 'X');
...
`andre@origamid.com
contato@origamid.com`.replace(regexpAB, 'X');
// X@origamid.com
...
// Procura: sequência de alfanuméricos
// no final da linha.
const regexpAEND = /\w+$/g;

`andre@origamid.com
contato@origamid.com`.replace(regexpAEND, 'X');
// andre@origamid.com
// contato@origamid.X
...
`andre@origamid.com
contato@origamid.com`.replace(regexpFM, 'X');
// andre@origamid.X
// contato@origamid.X

// Procura: sequência de alfanuméricos
// no início da linha.
const regexpFMS = /^\w+/gm;

`andre@origamid.com
contato@origamid.com`.replace(regexpFMS, 'X');
// X@origamid.com
// X@origamid.com
...
`andre@origamid.com\ncontato@origamid.com`.replace(regexpLF, '---');
// andre@origamid.com---contato@origamid.com

`andre@origamid.com
contato@origamid.com`.replace(regexpLF, 'X');
// andre@origamid.com---contato@origamid.com
...
'andre@origamid.com ©'.replace(regexpUnicode, '---');
// andre---origamid.com ---
